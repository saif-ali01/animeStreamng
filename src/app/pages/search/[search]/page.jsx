    ...
    return (
        <div className={`text-white ml-60 w-screen`}>
            ...
            <div className={`w-full mt-5`}>
                {
                    params.search === "audio" ? <div className="mt-3 grid xl:grid-cols-3  sm:grid-cols-2 gap-4">{numbers.map((number) => (
                        <Link href={"/pages/download/audio/111"}><AudioCard key={number} data={dummyData} /></Link>
                      ))}</div>
                       : params.search === "video" ? <div className="mt-3 grid xl:grid-cols-3  sm:grid-cols-2 gap-4">{numbers.map((number) => (
                        <Link href={"/pages/download/video/111"}><Card key={number} dummyData={videoData} /></Link>
                      ))}</div> : params.search === "wallpaper" ? <div className="mt-3 grid xl:grid-cols-3  sm:grid-cols-2 gap-4">{numbers.map((number) => (
                       <Link href={"/pages/download/wallpaper/111"}><WallpaperCard key={number} dummyData={wallpaperData} /></Link> 
                      ))}</div> : params.search=== "category" ? <div className="mt-5 grid xl:grid-cols-3  sm:grid-cols-2 gap-4">
                        {numbers.map((number) => (
                            <TrendingCard key={number}  data={slide} />
                    ))}
                      </div> :params.search === "raw" ? <div className="mt-3 grid xl:grid-cols-3  sm:grid-cols-2 gap-4">{numbers.map((number) => (
                        <Link href={"/pages/download/video/111"}><Card key={number} dummyData={rawData} /></Link>
                      ))}</div> :params.search === "flowframe" ? <div className="mt-3 grid xl:grid-cols-3  sm:grid-cols-2 gap-4">{numbers.map((number) => (
                        <Link href={"/pages/download/video/111"}><Card key={number} dummyData={flowFrameData} /></Link>
                      ))}</div> :<h1>not found</h1>
                }
            </div>
